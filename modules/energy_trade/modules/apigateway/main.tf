...
resource "aws_api_gateway_integration" "get_trade_history" {
  ...
  type                    = "AWS_PROXY"
  credentials             = var.apigateway_user_role_arn
...
resource "aws_api_gateway_integration" "admin_get_trade_history" {
  ...
  integration_http_method = "POST"
  type                    = "AWS_PROXY"
  credentials             = var.apigateway_admin_role_arn
...
resource "aws_api_gateway_deployment" "this" {
  ...
  depends_on = [
    aws_api_gateway_method.get_trade_history,
    aws_api_gateway_method.get_trade,
    aws_api_gateway_method.post_trade,
    aws_api_gateway_method.admin_get_trade_history,
    aws_api_gateway_method.admin_get_trade,
    aws_api_gateway_method.admin_delete_trade
  ]
}
...
resource "aws_apigatewayv2_route" "ws_connect" {
  ...
  authorizer_id = aws_apigatewayv2_authorizer.ws_request_authorizer.id
}
...
resource "aws_apigatewayv2_authorizer" "ws_request_authorizer" {
  api_id          = aws_apigatewayv2_api.ws_api.id
  authorizer_type = "REQUEST"
  authorizer_uri  = "arn:aws:apigateway:${var.region}:lambda:path/2015-03-31/functions/${var.ws_authorizer_lambda_arn}/invocations"
  identity_sources = ["route.request.header.Authorization"]
  name            = "WsRequestAuthorizer"
}
